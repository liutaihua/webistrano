...
#    @projects = Project.find(:all, :order => 'name ASC')
    @projectAll = Project.find(:all)
    session[:pid] = params[:id]

    respond_to do |format|
      format.html { render :layout => 'applicationopindex'}# index.rhtml
  ...
  def show
    ...
    respond_to do |format|
      #format.html {render 'show_scmer  if current_user.scmer?}# show.rhtml
      format.html {render :layout => 'applicationop' }# show.rhtml
      format.xml  {render :xml => @projectAll.to_xml }
    end
  end
