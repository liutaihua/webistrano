...
module Webistrano
  class Gitutils
    ...
    include Capistrano::CLI::Execute, Capistrano::CLI::Options,Git
    ...
    def revert_task!
      ...
      case execute!
      when false
	logger.info("*** Git Revert Error ****")
        deployment.complete_with_error!
        false
      else
	logger.info("*** Git Revert  finished ***")
    ...
    def execute!
     ...
     def execute_revert_action(config)
	arr_logs = []
	trevert = false;
	git_proj = config.git_project_name
	git_repo_host = "git@repo.op.sdo.com"
      	logger.trace("***Revert tag:.....#{@deployment.revision}#")
	retag = @deployment.revision  
      	logger.trace("***Go git clone .....#{git_repo_host}:#{git_proj} \n")
	Git.clone("#{git_repo_host}:#{git_proj}","/tmp/#{git_proj}") if FileUtils.rm_rf("/tmp/#{git_proj}")
	g=Git.open(working_dir="/tmp/#{git_proj}")
	g.log.each {|l| arr_logs.push l.sha }
	retag_hash = g::lib.tag_sha(retag)
	#arr_logs.each{|t| logger.trace t}
	
	if arr_logs.include?(retag_hash) then
	  logger.trace "***The Tag\{#{retag}\} >>> #{retag_hash} is found" 
	  arr_logs.each{|t| 
	   break  if t == retag_hash 

	   g::lib.sha_tag(t).each{ |tag|
	    logger.trace "***Add bad tag(HASH): " + t + " for project(#{git_proj})"
	    Gitinfor.find_or_create_by_commit_hash_and_tag_name_and_project_name(t,tag,git_proj)
           }
	    logger.trace "***Git Revert >>> " + t
	    trevert = true
	    g::lib.revert(t)
	   #g::lib.revert(t)
	  }
	 
	 n=Git.open(working_dir="/tmp/#{git_proj}")
	 if n::lib.diff_full('HEAD',retag_hash) == "" then
	  logger.trace "***git diff -p HEAD #{retag_hash} --- OK" 
	  if trevert then
	   n::lib.commit_amend("Revert Tag >>> #{retag}") 
	   logger.trace "***Git Push Remote  ........... #{git_repo_host}:#{git_proj}" 
	   n::lib.push("origin","master")
	   logger.trace "***Git Push Remote  ........... finished" 
          end
         else
	  logger.trace "***git diff -p HEAD #{retag_hash} --- Fail" 
         end
	else
	 logger.trace "***The Tag\{#{retag}\} HASH not found!" 
	end
