...
module Git
  ...
  class Lib
    ...
    def sha_tag(hash_string)
     command('show-ref',  ["--tags"],true,"|grep #{hash_string}").split("\n").collect do |res|
      if res =~/refs\/tags\/(.*)/ then
      $1
      end
     end
    end  

    def revert(tag_or_hash)
      command('revert', ['--no-edit', tag_or_hash])
    end 
